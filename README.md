# @osjwnpm/quas-debitis-praesentium - React Components for OpenLayers 6+

![logo](https://raw.githubusercontent.com/mmomtchev/@osjwnpm/quas-debitis-praesentium/master/@osjwnpm/quas-debitis-praesentium-logo.svg)

[![License: ISC](https://img.shields.io/github/license/mmomtchev/@osjwnpm/quas-debitis-praesentium)](https://github.com/osjwnpm/quas-debitis-praesentium/blob/master/LICENSE)
[![npm version](https://img.shields.io/npm/v/@osjwnpm/quas-debitis-praesentium)](https://www.npmjs.com/package/@osjwnpm/quas-debitis-praesentium)
[![Node.js CI](https://github.com/osjwnpm/quas-debitis-praesentium/workflows/Node.js%20CI/badge.svg)](https://github.com/osjwnpm/quas-debitis-praesentium/actions?query=workflow%3A%22Node.js+CI%22)
[![codecov](https://codecov.io/gh/mmomtchev/@osjwnpm/quas-debitis-praesentium/branch/master/graph/badge.svg)](https://codecov.io/gh/mmomtchev/@osjwnpm/quas-debitis-praesentium)
[![downloads](https://img.shields.io/npm/dm/@osjwnpm/quas-debitis-praesentium)](https://www.npmjs.com/package/@osjwnpm/quas-debitis-praesentium)
...
_@osjwnpm/quas-debitis-praesentium_ is an opinionated set of _React_ components for _OpenLayers_.
...
[pigeon-maps](https://pigeon-maps.js.org/) - [react-leaflet](https://react-leaflet.js.org/) - [@osjwnpm/quas-debitis-praesentium](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/)

Among the completely free and open source alternatives for creating maps with _React_, on a scale going from the most light-weight to the most feature-rich solution, _@osjwnpm/quas-debitis-praesentium_ is the right-most one.
...
```bash
npm --save install @osjwnpm/quas-debitis-praesentium ol react react-dom
```
...
| @osjwnpm/quas-debitis-praesentium          | Unit-tested _OpenLayers_ versions                                                                                                      | Unit-tested _React_ versions                |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
...
When using dynamic styles with React 18, you may get a warning in the console in debug mode: https://github.com/osjwnpm/quas-debitis-praesentium/issues/40. You can safely ignore it as has no functional consequences - React 18, including the concurrent renderer, is fully supported.

## Usage

_@osjwnpm/quas-debitis-praesentium_ is a set of reusable _React_ components that can be nested in various ways to create map applications for the web through _React_ composition in the true **spirit of _React_**.
...
In order to avoid confusion between the _OpenLayers_ classes and the _@osjwnpm/quas-debitis-praesentium_ classes which sometimes have the same names - all _@osjwnpm/quas-debitis-praesentium_ classes are prefixed with **R**. If a class begins with **R**, it is from _@osjwnpm/quas-debitis-praesentium_, otherwise it is an _OpenLayers_ class.
...
This is the simple overlay example - <https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/overlays>
...
import {RMap, ROSM, RLayerVector, RFeature, ROverlay, RStyle} from '@osjwnpm/quas-debitis-praesentium';
...
Check [examples/static_pages.html](https://github.com/osjwnpm/quas-debitis-praesentium/tree/master/examples/static_page.html) for a fully self-contained static HTML page using _@osjwnpm/quas-debitis-praesentium_.

You can also check the GPLed [XC-DB](https://github.com/mmomtchev/xc-db.git) for a larger and more complex project entirely implemented using React, Redux and @osjwnpm/quas-debitis-praesentium.

#### Contexts

Composition works by using _React_ Contexts. Every nested element uses the context of its nearest parent.

The underlying OpenLayers objects can be accessed using the `useOL()` hook - check the [`Geolocation`](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/geolocation) example to see how.
...
Additionally, `useRLayersComponent()` allows retrieving the containing _@osjwnpm/quas-debitis-praesentium_ component.
...
-   In an event handler that is a normal function and not an arrow lambda, `this` will contain the _@osjwnpm/quas-debitis-praesentium_ component and `this.context` will contain the context - this is an alternative to using `useOL()`
-   In all event handlers, _OpenLayers_ will pass the target object in `event.target` and the map in `event.map` - [the popups example](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/popups) uses this method
-   And finally, accessing arbitrary elements, even outside their contexts, is possible by using React references - `React.RefObject`s. [The high performance example](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/igc) contains an example of this. The underlying _OpenLayers_ objects can be accessed through the `ol` property of every component. Additionaly, for `layer` objects, the underlying _OpenLayers_ source can be accessed through `source`:
    ```ts
...
Style definitions can be placed anywhere inside the DOM and can be referenced with a _React_ reference. @osjwnpm/quas-debitis-praesentium includes two special types for dealing with styles:
...
-   <https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/features> for a basic example with static styles;
-   <https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/vectortiles> for a more complete example with dynamic styles;
-   <https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/cluster> for the use of caching.
...
-   [Clustering](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/cluster) runs the styling function every time the map is panned or zoomed
-   [Drop a pin](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/pindrop) runs code on every `pointermove` that carefully avoids rerendering
-   [Geo data](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/geodata) updates components at every `pointerenter`/`pointerleave`
-   [The high performance example](https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/#/igc) is a complex example that runs lots of code and updates components at every `pointermove`

Also, when searching for features listening on `pointermove`/`pointerenter`/`pointerleave` events, _@osjwnpm/quas-debitis-praesentium_ 2.0.0 and later, is able to eliminate very early feature layers that do not contain features listenening for those events. If your map contains a large number of features, and only a handful of these use `pointermove` events - try to group them in a separate layer.

## Examples

The examples can be found here:
<https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/>
...
```js
const withTranspile = require('next-transpile-modules')(['ol', '@osjwnpm/quas-debitis-praesentium']);
module.exports = withTranspile({experimental: {esmExternals: 'loose'}});
```

It is known to work with Next.js 10 to Next.js 13. You can check [`@osjwnpm/quas-debitis-praesentium-npm-tests`](https://github.com/osjwnpm/quas-debitis-praesentium-npm-tests) repository for examples for working configurations.
...
Currently, server-side rendering of raster layers on fixed map sizes has reached POC status and an online demo is accessible at https://@osjwnpm/quas-debitis-praesentium-ssr.meteo.guru/.
The code can be found in the `ssr` branch of this project. The `next.js` project can be found at <https://github.com/osjwnpm/quas-debitis-praesentium-ssr-demo.git>. _This is still not a user-friendly, install-and-run project._ Take a look at `pages/index.js` if you want see how it is meant to be used.
...
You can browse the full documentation at <https://mmomtchev.github.io/@osjwnpm/quas-debitis-praesentium/api>.
